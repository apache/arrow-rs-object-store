...
pub struct AmazonS3Builder {
    ...
    /// When set to true, dual-stack endpoints are used
    use_dualstack_endpoint: ConfigValue<bool>,
...
pub enum AmazonS3ConfigKey {
    ...
    ///
    /// See [`AmazonS3Builder::with_use_dualstack_endpoint`] for details.
    ///
    /// Supported keys:
...
impl AsRef<str> for AmazonS3ConfigKey {
    fn as_ref(&self) -> &str {
        match self {
            ...
            Self::UseDualstackEndpoint => "aws_use_dualstack_endpoint",
...
impl FromStr for AmazonS3ConfigKey {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            "aws_use_dualstack_endpoint" | "use_dualstack_endpoint" => {
                Ok(Self::UseDualstackEndpoint)
            }
...
impl AmazonS3Builder {
    ...
    /// * `AWS_USE_DUALSTACK_ENDPOINT` -> set to "true" to use dual-stack endpoints
    ...
    pub fn with_config(mut self, key: AmazonS3ConfigKey, value: impl Into<String>) -> Self {
        match key {
            ...
            AmazonS3ConfigKey::UseDualstackEndpoint => self.use_dualstack_endpoint.parse(value),
    ...
    pub fn get_config_value(&self, key: &AmazonS3ConfigKey) -> Option<String> {
        match key {
            ...
            AmazonS3ConfigKey::UseDualstackEndpoint => {
                Some(self.use_dualstack_endpoint.to_string())
            }
    ...
    ///
    /// Defaults to `false`. If enabled for a service or region without a dual-stack endpoint,
    /// requests may fail. This option only affects endpoints generated by the builder; explicitly
    /// configured endpoints are left unchanged.
    pub fn with_use_dualstack_endpoint(mut self, use_dualstack_endpoint: bool) -> Self {
        self.use_dualstack_endpoint = use_dualstack_endpoint.into();
        self
    }
    ...
    pub fn build(mut self) -> Result<AmazonS3> {
        ...
        let (session_provider, zonal_endpoint) = match self.s3_express.get()? {
            true => {
                ...
                let endpoint = s3_express_endpoint(&bucket, zone, &region, use_dualstack_endpoint);
        ...
        // AWS supports path-style and virtual-hosted-style endpoint formats:
        ...
        let bucket_endpoint = match (&endpoint, zonal_endpoint, virtual_hosted) {
            ...
            (None, None, false) => {
                format!("https://s3{dualstack}.{region}.amazonaws.com/{bucket}")
            }
        };
...
mod tests {
    ...
    fn s3_test_config_from_map() {
        ...
        let options = HashMap::from([
            ...
            ("aws_use_dualstack_endpoint", "true".to_string()),
        ...
        assert!(builder.use_dualstack_endpoint.get().unwrap());
        assert_eq!(
    ...
    fn s3_test_config_get_value() {
        ...
        let builder = AmazonS3Builder::new()
            ...
            .with_config(AmazonS3ConfigKey::UseDualstackEndpoint, "true")
        ...
        assert_eq!(
            builder
                .get_config_value(&AmazonS3ConfigKey::UseDualstackEndpoint)
                .unwrap(),
            "true"
        );
        assert_eq!(
    ...
    fn test_s3_express_endpoint() {
        assert_eq!(
            s3_express_endpoint(
                "bucket-base-name--use1-atl2-az1--x-s3",
                "use1-atl2-az1",
                "us-east-1",
                false
            ),
            "https://bucket-base-name--use1-atl2-az1--x-s3.s3express-use1-atl2-az1.us-east-1.amazonaws.com"
        );
        assert_eq!(
            s3_express_endpoint(
                "bucket-base-name--use1-az4--xa-s3",
                "use1-az4",
                "us-east-1",
                true
            ),
    ...
    #[cfg(feature = "reqwest")]
    #[test]
    fn test_generated_s3_endpoints() {
        let cases = [
            (
            ...
            (
                AmazonS3Builder::new()
                    .with_bucket_name("bucket-base-name--use1-atl2-az1--x-s3")
                    .with_s3_express(true),
                "https://bucket-base-name--use1-atl2-az1--x-s3.s3express-use1-atl2-az1.us-east-1.amazonaws.com",
            ),
            (
                AmazonS3Builder::new()
                    .with_bucket_name("bucket-base-name--use1-az4--xa-s3")
                    .with_s3_express(true)
                    .with_use_dualstack_endpoint(true),
                "https://bucket-base-name--use1-az4--xa-s3.s3express-use1-az4.dualstack.us-east-1.amazonaws.com",
            ),
            (
                AmazonS3Builder::new()
                    .with_bucket_name("bucket")
                    .with_endpoint("http://generic-endpoint")
                    .with_use_dualstack_endpoint(true),
                "http://generic-endpoint/bucket",
            ),
            (
                AmazonS3Builder::new()
                    .with_bucket_name("bucket")
                    .with_config(AmazonS3ConfigKey::S3Endpoint, "http://s3-specific-endpoint")
                    .with_use_dualstack_endpoint(true),
                "http://s3-specific-endpoint/bucket",
            ),
        ];

        for (builder, expected) in cases {
            let s3 = builder.build().unwrap();
            assert_eq!(s3.client.config.bucket_endpoint, expected);
        }
    }
